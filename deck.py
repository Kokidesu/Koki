...
def divider(part,title,subtitle):
    s=slide(NAVY)
    rect(s,0.9,3.55,3.2,0.06,ACCENT)
    text(s,0.9,2.35,11.5,part,20,ACCENT,bold=True)
    text(s,0.9,2.75,11.5,title,34,WHITE,bold=True)
    text(s,0.9,3.8,10.5,subtitle,18,LIGHT,italic=True,ls=1.1)
    return s
...
# 1 — TITLE
s=add(slide(NAVY))
rect(s,0,3.05,SW_IN,0.06,ACCENT)
text(s,1.0,1.45,11.3,"TRANSNATIONAL CRIME & THE LIMITS OF INTERNATIONAL JUSTICE",16,ACCENT,bold=True,align="c")
text(s,1.0,1.95,11.3,"What the Epstein Case Reveals",40,WHITE,bold=True,align="c",h=1.1)
text(s,1.0,3.35,11.3,"A case study in cross-border exploitation, jurisdiction, and accountability",18,LIGHT,italic=True,align="c")
text(s,1.0,6.45,11.3,"Group Presentation  •  May 2026",13,GREY,align="c")

# 2 — AGENDA
s=add(slide()); header(s,"OVERVIEW","Our Central Question & Roadmap")
rect(s,0.6,1.6,12.1,1.25,LIGHT)
text(s,0.9,1.82,11.5,"Why is transnational sexual exploitation so hard to prosecute — and what do international law and cross-border cooperation need in order to work?",19,NAVY,bold=True,italic=True,ls=1.1,h=1.0)
bullets(s,0.7,3.25,12,[
...
text(s,0.7,6.75,12,"Note: This presentation centres victims and legal systems — not sensational detail.",13,GREY,italic=True)

# 3 — DIVIDER P1
add(divider("PART 1","Framing & the International Scope","Who Epstein was — and why this case crosses borders"))

# 4 — WHO WAS EPSTEIN
s=add(slide()); header(s,"PART 1  •  BACKGROUND","Who Was Jeffrey Epstein?")
bullets(s,0.7,1.6,7.4,[
...
],18,gap=16,bold_lead=True)
rect(s,8.4,1.7,4.3,4.9,LIGHT)
text(s,8.7,1.95,3.8,"WHY HE MATTERS HERE",14,ACCENT,bold=True)
text(s,8.7,2.5,3.8,"His death meant accountability had to be pursued through others — and across borders. The case became a test of how legal systems cooperate.",16,SLATE,italic=True,ls=1.18,h=3.8)
...
bw=2.85; gap=0.18; x0=0.7; y0=2.2
for i,(yr,cap) in enumerate(stages):
    x=x0+i*(bw+gap)
    rect(s,x,y0,bw,0.85,NAVY)
    text(s,x,y0+0.16,bw,yr,28,WHITE,bold=True,align="c")
    rect(s,x+bw/2-0.03,y0+0.85,0.06,0.35,ACCENT)
    text(s,x,y0+1.35,bw,cap,14,SLATE,ls=1.15,h=2.2)
text(s,0.7,5.9,12,"Across two decades the case moved from a local plea bargain to a global question about justice, wealth, and cross-border accountability.",15,NAVY,italic=True,ls=1.15,h=1.0)
...
rect(s,8.4,1.7,4.3,4.9,LIGHT)
text(s,8.7,1.95,3.8,"THE CORE TENSION",14,ACCENT,bold=True)
text(s,8.7,2.5,3.8,"The network was global, but criminal enforcement is national. That gap is the heart of our presentation.",16,SLATE,italic=True,ls=1.18,h=3.6)

# 7 — DIVIDER P2
add(divider("PART 2","The International Legal Framework","How international law addresses crime that crosses borders"))
...
text(s,0.7,6.55,12,"So the question is institutional, not just moral: can sovereign systems coordinate fast enough?",15,NAVY,bold=True,italic=True)
...
],19,gap=16,bold_lead=True)
text(s,0.7,6.55,12,"It gives 190+ states a shared vocabulary — a precondition for cooperation.",15,NAVY,italic=True)
...
text(s,0.7,6.55,12,"In short: the machinery for international cooperation already exists on paper.",15,NAVY,italic=True)
...
],19,gap=15,bold_lead=True)
text(s,0.7,6.7,12,"The tools exist — but speed and political will decide whether they work.",15,NAVY,italic=True)
...
],19,gap=17,bold_lead=True)
text(s,0.7,6.55,12,"Stick to what is adjudicated: charges, a conviction, a settled civil case — not speculation.",15,NAVY,italic=True)

# 13 — DIVIDER P3
add(divider("PART 3","Wealth, Mobility & Impunity","How resources can delay — though not always defeat — accountability"))
...
text(s,0.7,6.7,12,"None of this is unique to one man — it is how illicit elite wealth tends to behave.",15,NAVY,italic=True)

# 15 — ELITE IMPUNITY (compare)
s=add(slide()); header(s,"PART 3  •  COMPARISON","\"Elite Impunity\": 2008 vs 2019")
rect(s,0.7,1.7,5.85,4.7,PANEL)
text(s,1.0,1.95,5.3,"2008 — FLORIDA",16,ACCENT,bold=True)
bullets(s,1.0,2.5,5.3,[
...
rect(s,6.8,1.7,5.85,4.7,NAVY)
text(s,7.1,1.95,5.3,"2019 — FEDERAL (NY)",16,ACCENT,bold=True)
bullets(s,7.1,2.5,5.3,[
 "Federal sex-trafficking charges",
 "Held without bail",
 "Treated as serious organised wrongdoing",
 "Case ended only with his death in custody",
],15,LIGHT,gap=12)
text(s,0.7,6.6,12,"The same conduct, two very different responses — a decade apart.",15,NAVY,italic=True)
...
text(s,0.7,6.55,12,"Central question: would the UK, France or Japan have acted faster — or slower?",15,NAVY,bold=True,italic=True)

# 17 — DIVIDER P4
add(divider("PART 4","Accountability, Transparency & Lessons","Designing cross-border justice that actually protects victims"))
...
text(s,0.7,6.55,12,"Transparency is not automatically just — its design determines who it protects.",15,NAVY,italic=True)
...
],18,gap=15,bold_lead=False)
rect(s,8.4,1.7,4.3,4.9,LIGHT)
text(s,8.7,1.95,3.8,"JAPAN ANGLE",14,ACCENT,bold=True)
text(s,8.7,2.5,3.8,"Japan has an anti-trafficking action framework and is assessed annually in the US State Department's TIP Report — a useful benchmark for how a non-Western system measures up on cross-border cooperation.",15,SLATE,italic=True,ls=1.2,h=4.0)
...
for num,head_,sub in items:
    rect(s,0.7,y,0.8,0.95,ACCENT)
    text(s,0.7,y+0.2,0.8,num,24,WHITE,bold=True,align="c")
    text(s,1.65,y+0.04,10.9,head_,18,NAVY,bold=True)
    text(s,1.65,y+0.46,10.9,sub,14,SLATE,italic=True)
    y+=1.08
rect(s,0.6,6.05,12.1,1.15,LIGHT)
text(s,0.85,6.18,11.6,"SELECTED SOURCES & INSTRUMENTS",11,ACCENT,bold=True)
text(s,0.85,6.5,11.6,"Palermo Protocol (2000) · UN Convention against Transnational Organized Crime · US DOJ filings (US v. Epstein 2019; US v. Maxwell 2021) · US State Dept. Trafficking in Persons Reports · contemporary reporting on 2025 disclosures.",11,SLATE,ls=1.15,h=0.7)
