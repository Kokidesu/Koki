...
- 🧠 **ほのちゃん / Hono-chan** — `koki research "..."` asks a world-class research persona that searches the web, cross-checks sources, and answers with extreme specificity, citations, and a **confidence score**. Bilingual (JP/EN), uses the most capable model + extended thinking. **Remembers past research** (`~/.koki/hono_memory.jsonl`) so she gets smarter over time, and can be summoned inside `koki chat` with `ほのちゃん …` or `/hono …`.
...
# English works too — she answers in the language you ask in
koki hono "What was Japan's real GDP growth in 2025, and the main drivers?"

# chat の中からほのちゃんを呼ぶ
koki chat
#   you › ほのちゃん、リーマンショックの原因を具体的に
#   you › /hono explain the Calvin cycle accurately

# 賢さ最大化: 最上位モデルを指定（APIキーが対応している場合）
#   export KOKI_RESEARCH_MODEL=claude-opus-4-8
# 記憶ファイルの場所を変える: export KOKI_MEMORY_PATH=~/my-notes.jsonl
